class Api::SaleReportsController < ApplicationController
    ...
    def create
        if current_user.role.role_level.eql?('branch')
            if current_user.branch.sales.where(sale_date: Date.strptime(@from, '%m/%d/%Y')..Date.strptime(@to, '%m/%d/%Y')).exists?
                @title = "#{Date.strptime(@from, '%m/%d/%Y').strftime('%B %d, %Y')} - #{Date.strptime(@to, '%m/%d/%Y').strftime('%B %d, %Y')}"
                @sale_report = current_user.branch.brand.sale_reports.where( title: @title, date_from: Date.strptime(@from, '%m/%d/%Y'), date_to: Date.strptime(@to, '%m/%d/%Y'), total_sales: current_brand.sales.where(sale_date: Date.strptime(@from, '%m/%d/%Y')..Date.strptime(@to, '%m/%d/%Y')).pluck(:net_total_sales).sum, num_of_days: (Date.strptime(@to, '%m/%d/%Y') - Date.strptime(@from, '%m/%d/%Y')).to_i ).first_or_create
