class Api::SaleReportsController < ApplicationController
    ...
    def create
        if current_brand.sales.where(sale_date: Date.strptime(@from, '%m/%d/%Y')..Date.strptime(@to, '%m/%d/%Y')).exists?
            ...
            @sale_report = current_user.sale_reports.where( title: @title, date_from: Date.strptime(@from, '%m/%d/%Y'), date_to: Date.strptime(@to, '%m/%d/%Y'), brand_id: current_brand.id, total_sales: current_brand.sales.where(sale_date: Date.strptime(@from, '%m/%d/%Y')..Date.strptime(@to, '%m/%d/%Y')).pluck(:net_total_sales).sum, num_of_days: (Date.strptime(@to, '%m/%d/%Y') - Date.strptime(@from, '%m/%d/%Y')).to_i ).first_or_create
            render json: { saleReport: @sale_report, status: 200 }
        else
            render json: {saleReport: false, status: 200 }
        end        
    end
