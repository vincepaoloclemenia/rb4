class PurchaseItem < ActiveRecord::Base
	...
	def item_total_vat
		return vat_type == "VAT-Exempted" ? 0.00 : (purchase_item_total_amount - (purchase_item_total_amount / purchase.branch.percentage_tax[:inc]).round(2)).round(2) 
	end

	def item_total_net
		return vat_type == "VAT-Exempted" ? item_total_amount : (purchase_item_total_amount / purchase.branch.percentage_tax[:inc]).round(2)
	end
